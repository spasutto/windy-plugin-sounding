...
export class Cumulus extends PureComponent {
  render({ x, y, scale = 1, opacity = 1, fillOpacity = 1, text = "" }) {
    const parts = [];
    parts.push(
        <path
          class="cumulus" style={`fill-opacity: ${fillOpacity}`}
    ...
    if (text.length > 0) {
      parts.push(<text x="7" y="21" style="font-family:Verdana;font-size:8;stroke-width:0;fill:#000;">{text}</text>);
    }
    return <g transform={`translate(${x - 36}, ${y - 28}) scale(${scale})`} style={`opacity: ${opacity}`}>{parts}</g>;
  }
}
